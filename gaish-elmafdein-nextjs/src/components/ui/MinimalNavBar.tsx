...
export default function MinimalNavBar({ locale, className = '' }: Props) {
  ...
  const langPath = (code: string) => {
    const current = pathname ?? '/';
    // Remove leading locale segment like /ar or /en (only at the start)
    const withoutLocale = current.replace(/^\/[a-z]{2}(?=\/|$)/, '');
    const tail = withoutLocale.length ? withoutLocale : '/';
    return `/${code}${tail}`;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-4">
        ...
        <div className="relative">
          <button
            ...
            aria-haspopup="true" aria-expanded={openLang ? 'true' : 'false'}
          >
